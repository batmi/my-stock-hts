...
[대사의 사각 · 토스 · 2026-09-09] 토스 Open API 1.2.15 가 조회 범위를 명문화했다 —
 **Open API 로 낼 수 있는 호가 유형(지정가·시장가·장마감지정가)의 주문만** 목록·상세에
 나온다. 시간외단일가·장전/장후 시간외 종가로 접수된 주문은 `OPEN`·`CLOSED` 목록에도,
 주문 상세 조회에도 **아예 나오지 않는다.**
 우리가 내는 주문은 전부 지원 유형이라 정상 경로는 영향이 없다. 다만 운용자가 토스 앱에서
 시간외 주문을 직접 냈다면 그 주문은 여기서 보이지 않는다 — 대사 결과의 '없음'을
