...
def _nxt_quote_window():
    ...
    KRX 애프터마켓이 대표 시장이고(J 현재가), 15:30~16:00 은 시장이 없는 휴게다
    (NXT 는 프리마켓만 운영한다). 야간·휴장일도 생략한다.
    """
...
def _nxt_quote_phase():
    """국내 연장거래 시세 처리 단계를 '한 번의 휴장 판정'으로 결정한다(중복 휴장조회 방지).
       ...
       'break'     : 15:30~16:00 — KRX 휴게. 어느 시장도 열려 있지 않다(NXT 는 프리마켓만 운영).
                     KRX 정규장 종가에 고정한다(NX 조회·회상 없음 — 실측 2026-09-14 16:10 SK하이닉스
                     NX 가 KRX 와 다른 값을 돌려줬다. 그 값의 출처를 알 수 없으니 쓰지 않는다)
