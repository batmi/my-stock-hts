...
전파하는 패키지다. 여기 둘을 그 안에 넣으면 이름이 부딪힌다(toss_api 54개 중 12개, realtime
50개 중 7개). 그중 `get_access_token` 은 **KIS 토큰과 토스 토큰**이라 의미까지 정면으로
충돌한다 — `api.get_access_token` 이 어느 쪽인지 모호해지고 `patch.object(api, ...)` 가
엉뚱한 클라이언트를 덮는다. 평탄화가 필요한 계층이 아니므로(호출부는 늘 `toss_api.X` 로
쓴다) 옆에 둔다.
