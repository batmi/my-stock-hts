...
# [실측 2026-08-05] 이 값을 낮춰 봤지만(0.25=5 TPS) **거부는 줄지 않고 분석만 느려졌다.**
#  거부 시점의 클라이언트 실제 전송률을 찍어 보니 결정적이었다 —
#    · 첫 거부: 전송 9건/초, 그때 실효 한도는 18.2  → **한도가 제약이 아니었다**
#    · 이후 한도를 5.69까지 내려도 거부 지점은 그대로 7~10건/초
#  즉 우리는 우리 한도에 닿기도 전에 거부당한다. 한도를 내리는 것은 거부에 아무 영향이 없고
#  (도달하지 않는 값이므로) 수요가 많을 때 처리량만 깎는다. 단일 프로세스는 확인했고
#  (`ps aux | grep main.py`), 어댑터 재시도도 status_forcelist=[] 라 상태코드 재전송은 없다.
#  원인은 우리 전송량이 아니라 그 밖에 있다(계정 실제 유량 등). 그러므로 **하한은 처리량을
#  지키는 쪽으로 되돌린다.** 부하를 줄이려면 한도가 아니라 호출 수 자체를 줄여야 한다
#  (후보당 호가 REST 1콜 × 관심종목 수 — WS 등록 41건 한도가 현재가로 다 차서 호가는 0개다).
REAL_TPS_SAFETY_MIN = 0.85   # 백오프 하한 (= 20 * 0.85 = 17 TPS). 과도한 하강 방지

# [TPS 우선순위] 조회성(메뉴) 호출이 쓸 수 있는 한도의 비율. 시스템 트레이딩은 항상 1.0.
#  매매 판단·주문은 시각이 곧 가격이라 미룰 수 없지만 조회 메뉴는 몇 초 늦어도 무방하다.
#  종전에는 모든 스레드가 같은 토큰 버킷을 동등하게 다퉈, 메뉴 1·2를 여는 동안 후보 분석
#  스레드가 EGW00201로 '최종 실패'했다(2026-08-05 관측). 실패한 조회는 재시도로 끝나는 게
#  아니라 그 종목의 매수·매도 판정을 그 주기에서 통째로 건너뛰게 만든다.
#  한도를 미리 쪼개 놓는 것이 아니라 조회 쪽 '문턱'만 낮추는 방식이라, 매매가 한가한
#  시간대에는 메뉴가 한도를 그대로 다 쓴다(유휴 손실 없음).
#  1.0으로 두면 종전처럼 동등 경쟁한다.
LOW_PRIORITY_TPS_SHARE = 0.5
