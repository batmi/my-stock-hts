...
class GlobalSettings(BaseModel):
    ...
    RISK_SCALING_PARAMS: dict = {
        ...
        #    룩백 60/180/전체도 모두 효율 0.05~0.07 (기준·배수·룩백 어느 축으로도 개선 안 됨)
        #  [해석] MDD는 확실히 준다(29/30). 그러나 **위험조정수익(MAR)은 26/30에서 악화**했다.
        #   낙폭 1%p를 줄이는 데 수익 20%p를 쓰는 구조다(효율 0.05). 같은 방법론으로 휩소율
        #   최소배수를 정할 때 채택 기준이 0.70이었고 0.13은 '방어 대비 비용이 나쁘다'며
        #   기각했다(WHIPSAW_MIN_SCALE 주석). 그 잣대로는 모든 드로다운 설정이 미달이다.
        #  [왜 비싼가] 드로다운 축은 낙폭이 **난 뒤에** 노출을 줄인다. 추세추종에서는 그
        #   구간이 회복 구간과 겹쳐 반등을 놓친다. 게다가 이미 82%의 날을 다른 두 축이
        #   줄이고 있어 곱하면 유휴현금이 55%→63%까지 오른다(자본이 놀아 기회를 잃는다).
        #  [그래도 남기는 이유] 꼬리는 실제로 지킨다 — 최악 시행 MDD -34.7% → -26.5%.
        #   '자본대비 리스크에 한도를 둔다'는 원칙의 마지막 층이라 제거하지 않고, 꼬리 방어의
        #   절반(-34.7→-29.9)을 비용의 45%로 사는 0.90/0.80으로 완화했다. MAR만 보면 OFF가
        #   최적이므로(5.53 vs 4.83), 수익 우선이면 USE_DRAWDOWN_RISK_SCALING=False가 맞다.
