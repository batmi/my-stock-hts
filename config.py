...
# [실측 2026-08-05] 하한을 0.85(=17 TPS)로 두면 AIMD가 수렴할 수 없다. 거부 시점의 클라이언트
#  실제 전송률을 찍어 보니 **초당 8~9건에서도 EGW00201이 났고**(라즈베리파이, 단일 프로세스로
#  `ps aux | grep main.py` 확인), 그 뒤 모든 백오프 로그가 '(하한 도달)'이었다. 즉 실제 허용치가
#  하한보다 낮아, 아무리 물러나도 계속 걸리는 상태로 고정됐다. **바닥이 실제 한도보다 높으면
#  적응 제어는 구조적으로 무의미하다.**
#  하한은 '운영점'이 아니라 '안전망'이어야 한다 — 낮춰 두면 컨트롤러가 서버가 허용하는 지점을
#  스스로 찾고, 여유가 생기면 가산 증가로 (윈도우당 한 단계씩) 다시 올라간다. 낮은 값에
#  머무르는 것이 아니라 '내려갈 수 있게' 하는 변경이다.
REAL_TPS_SAFETY_MIN = 0.25   # 백오프 하한 (= 20 * 0.25 = 5 TPS). 폭주 방지용 안전망
