...
class GlobalSettings(BaseModel):
    ...
    RISK_SCALING_PARAMS: dict = {
        ...
        #    축 없음          56.1  -17.8  3.14   70.7      —      —      —      —     —    -22.7
        #    구 5/10 .75/.5   35.5  -16.8  2.19   74.1  +0.52 -15.31  16/30   6/30  0.03   -20.3
        #    5/10 .90/.80     44.0  -15.6  2.73   72.3  +1.50  -5.25  23/30  12/30  0.29   -21.6  ← 채택
        #    5/10 .50/.25     22.7  -15.9  1.61   77.4  +2.29 -28.26  22/30   3/30  0.08   -18.6
        #    3/6  .75/.5      31.0  -14.7  2.12   76.9  +2.87 -27.50  27/30   3/30  0.10   -18.4
        #    룩백 60/180/전체  효율 0.11~0.14 (룩백을 늘려도 0.90/0.80만 못하다)
        #  [해석] 채택안(0.90/0.80)이 모든 후보 중 MAR·효율이 가장 좋다. 구 설정(0.75/0.5)은
        #   MDD를 16/30에서만 줄이면서 수익은 15%p 깎아 효율 0.03으로 최하위였다 — 강하게
        #   조일수록 나빠지는 구간이다. 다만 축을 아예 빼면 MAR이 더 좋으므로(3.14 vs 2.73),
        #   이 축은 여전히 '위험조정수익을 사는' 장치가 아니라 '꼬리를 사는' 장치다.
        #  [왜 비싼가] 드로다운 축은 낙폭이 **난 뒤에** 노출을 줄인다. 추세추종에서는 그
        #   구간이 회복 구간과 겹쳐 반등을 놓친다. 게다가 이미 82%의 날을 다른 두 축이
        #   줄이고 있어 곱하면 유휴현금이 70.7%→72.3%로 더 오른다.
        #  [그래도 남기는 이유] 꼬리를 지킨다 — 최악 시행 MDD -22.7% → -21.6%.
        ...
        #  ※ 이 표는 portfolio_backtest의 트레일링 반납 상한 누락을 고친 뒤 다시 측정한 값이다.
        #    수정 전 수치(수익 146.5%·효율 0.05 등)는 청산이 늦게 잡힌 낙관 편향이었다
        #    (같은 백테스트가 3년 수익을 중앙 +82.8%p 과대평가). 결론(0.90/0.80 채택)은
        #    바뀌지 않았지만 근거 수치는 전부 교체했다.
