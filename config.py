...
INDICES_GROUPS = {
    ...
    "3": {"name": "섹터 및 주요 지표 (Sectors & Key Indicators)", "indices": ["London - Samsung GDR", "SOX (반도체)", "VIX (변동성)", "MSCI 신흥국", "하이일드 채권"]},
...
SELL_STRATEGY = {
    ...
    # [역매수 유예 기간 허용 손실률]
    # 역매수로 진입한 종목이 시간 청산 유예 기간(TIME_STOP_DAYS) 동안 최대로 버틸 수 있는 손실 제한입니다.
    ...
    # [슈퍼 모멘텀 익절 RSI 기준]
    # 현재 퀀트 점수가 매우 높고 신고가 근처에서 추세를 유지 중(슈퍼 모멘텀 상태)일 때,
    # 이익 실현을 지연시키기 위해 상향 조정되는 RSI 기준입니다.
    "SUPER_TAKE_PROFIT_RSI": 85.0,
    ...
    # 1. 발동 조건: 수익률이 이 값 이상 도달해야 트레일링 스탑 감시가 시작됩니다. (기본값: 15.0%)
    #    (※ 팁: 이 값을 '익절 기준'보다 낮게 잡으면 추세 추종형, 높게 잡으면 목표 달성형이 됩니다.)
    "TRAILING_STOP_ACTIVATION_RATE": 15.0,
    
    # 2. 매도 조건: 최고가 대비 이 비율만큼 하락하면 이익 실현 매도를 수행합니다. (기본값: 4.0%)
    "TRAILING_STOP_CALLBACK_RATE": 4.0,
    ...
    # [추가] ATR 손절률 최대 한도
    # 데이터 오류나 과도한 변동성으로 인해 손절폭이 비정상적으로 커지는 것을 방지합니다.
    # (기본값: -15.0%, 0으로 설정 시 미사용)
...
def load_dynamic_config():
    ...
    if os.path.exists(config_path):
        try:
            ...
            global AUTO_MORNING_BRIEFING_USE, AUTO_MORNING_BRIEFING_TIME
            global SCREEN_DEBUG_LEVEL, FILE_DEBUG_LEVEL
            global SYSTEM_MAX_CONSECUTIVE_ERRORS, SYSTEM_TRADING_START_TIME, SYSTEM_TRADING_END_TIME, SYSTEM_RISK_PER_TRADE
            ...
            if "AUTO_MORNING_BRIEFING_USE" in data: AUTO_MORNING_BRIEFING_USE = data["AUTO_MORNING_BRIEFING_USE"]
