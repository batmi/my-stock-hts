...
# ==========================================================
# 관심종목 현재가/체결강도를 백그라운드에서 주기적으로 마이크로 캐시에 채워,
# '시장 지수 조회'/'종목 시세 분석' 메뉴 진입 시 임계경로 API 호출 없이 즉시 표시되게 한다.
#  - 개요 화면은 약간(수십 초)의 지연을 허용하므로, 예열 데이터를 OVERVIEW_PRICE_TTL_SEC 동안 재사용한다.
#  - 모의투자(2 TPS)는 예열이 시스템 트레이딩 호출과 TPS를 다투므로 기본 비활성화한다(실전만 ON).
OVERVIEW_WARM_ENABLED = True          # 백그라운드 예열 사용 여부 (실전 계좌에서만 자동 동작)
OVERVIEW_WARM_INTERVAL_SEC = 15       # 예열 주기(초). OVERVIEW_PRICE_TTL_SEC 보다 작아야 함
OVERVIEW_PRICE_TTL_SEC = 25           # 개요 화면이 예열 데이터를 재사용하는 최대 허용 시간(초)
