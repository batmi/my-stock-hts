...
class GlobalSettings(BaseModel):
    ...
    SELL_STRATEGY: dict = {
        ...
        #  [적용 후보] analyze_sell 의 트레일링 판정만 마감 직전(예: 15:20) 1회로 옮기고
        #   손절은 지금처럼 즉시 집행.
        #  [브래킷 검증 · 같은 날] "종가 팔은 그날 종가를 미리 안다"는 반론을 집행 쪽에서
        #   지웠다 — 청산을 **다음 거래일 시가**로 미뤄 하룻밤 갭을 그대로 맞게 한 팔을 넣었다.
        #   실제 15:20 집행은 종가 체결과 익일 시가 체결 **사이**이므로 두 끝이 다 이기면 그
        ...
        #     (15:20에는 종가를 모른다). 그건 분봉이 있어야 지운다 — 아래.
        ...
        #   → 적용안: analyze_sell 의 트레일링 판정만 마감 직전 1회로. 손절은 즉시 유지.
        #     아직 적용하지 않았다 — 운영 반영은 별도 결정.
