...
class GlobalSettings(BaseModel):
    ...
    #
    # [보류 1건] ADX 문턱 20 → 15. 하위4구간 32·35·28/60(3씨드), 전체 창 3씨드 모두 우세,
    #  수익도 모두 개선(+27·+27·+8)이다. 그러나 고변동 대조구간에서 3씨드 모두 대패한다
    #  (4-0-11 · 3-0-12 · 5-0-10). 반대 방향(25)은 정확히 거울상이다 — 평시 22/60로 지고
    #  고변동에서 이긴다. 국면 트레이드오프 손잡이라 고정값으로는 채택하지 않는다.
    #  국면 조건부 적용은 PROFIT_LOCK 재검토(2027년 초)와 함께 볼 것.
    #
    ...
    #
    # [진입 순위 — 결정은 견고, 격차는 과장이었다] 3씨드 × 게이트 OFF/ON = 6회 측정.
    #   추세품질 1순위(종전 실매매) : 26·22·28 (OFF) / 25·24·25 (ON) — 6회 전부 채택선 미달
    #   점수 → 추세품질(현행)       : 30·29·33 (OFF) / 34·29·31 (ON) — 평균 31.0, 경계선
    #  ① 1순위를 점수로 바꾼 결정은 오염과 무관하게 선다. 전체 창 수익도 3씨드 전부 열위다.
    #  ② 그러나 위에 적힌 '286.8% · 전체 4-0-11'은 재현되지 않는다. 재실행에서 실매매 순위의
    #     전체 창 성적은 8-0-7 · 2-0-13 · 5-0-10으로 흩어지고, 씨드에 따라 무승부에 가깝다.
    ...
    #     당일 재매수 0~1건)는 그대로 재현된다. 진단 자체는 오염과 무관하다.
    #
    ...
    #
    # [교훈] 이 오염은 채택선을 낮추는 것이 아니라 **대안 쪽만 깎는** 편향이었다. 그래서 오염된
    #  측정의 '기각'은 재확인이 필요하고(이번에 6팔이 되살아났다), 오염된 측정의 '채택'은 오히려
    #  보수적이다. 그리고 되살아난 6팔이 5씨드에서 전부 무너졌다는 사실은 씨드 규약이 오염보다
    #  강한 필터라는 뜻이다 — 경계선 결과 앞에서는 무엇보다 먼저 씨드를 늘릴 것.
