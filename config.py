...
REAL_TPS_SAFETY = 1.0        # 시작값 = 명목 한도(20 TPS). 서버가 거부하면 그때부터 내려간다
REAL_TPS_SAFETY_MAX = 1.0    # 상향 상한 = 명목 한도. 거부가 멎으면 여기까지 되올린다
REAL_TPS_SAFETY_MIN = 0.05   # 하한 = 20 * 0.05 = 1 TPS. 컨트롤러가 무릎을 만날 때까지
                             #  내려갈 수 있어야 한다(종전 17은 무릎 6의 3배라 도달 불가였다)
TPS_ADAPT_STEP = 0.05        # 윈도우(1초)당 가산 증가폭 (TPS). 완만히 되올린다
TPS_ADAPT_BACKOFF = 0.9      # EGW00201 발생 시 실효 TPS 곱셈 감소율 (×0.9)
                             #  20 → 6 까지 12회 백오프. 거부는 초당 수 건이라 수 초면 수렴한다
