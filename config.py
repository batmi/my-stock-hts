...
#
#  [국내]
#  매수: 위탁수수료만 (온라인 약 0.014%, 유관기관 제비용 포함해 0.015%로 올림)
#  매도: 위탁수수료 + 증권거래세. 0.23%는 종전 관찰모드·백테스트가 쓰던 값을 그대로
#        승계한 것이다(세율이 바뀌었다면 이 상수를 갱신할 것 — 코드 곳곳이 아니라 여기만).
...
#
#  [확인 필요] 아래 0.25%는 KIS 미국주식 온라인 위탁수수료의 통상값이다. 실제 적용
#   요율(우대·이벤트 요율 포함)을 확인하면 이 두 상수만 갱신하면 된다.
#   미국 매도분 SEC Fee·FINRA TAF는 요율이 0.001% 미만이라 여기 포함하지 않는다.
