...
class GlobalSettings(BaseModel):
    ...
    # [추가] 장 마감 후 국내 현재가·지표 기준 (True = KRX 정규장 종가 고정)
    # KRX 정규장(09:00~15:30) 밖에서는 NXT 프리마켓·애프터마켓 체결가가 '현재가'로 들어온다.
    # 이 값을 확정된 일봉 마지막 봉에 덮어쓰면 EMA·RSI·CCI·ATR·52주 위치가 함께 흔들리는데,
    # 시간외 거래량은 정규장의 수백분의 1이라 지표 기준으로는 노이즈에 가깝다.
    ...
    # True면 정규장 밖에서 차트·지표·화면 현재가를 KRX 정규장 종가로 고정한다.
    # False면 종전처럼 시간외(NXT) 체결가를 반영한다.
    # ※ 주문 가격 계산은 이 설정과 무관하게 항상 실시간가를 쓴다(NXT 시간대 체결 보장).
