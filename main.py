...
def show_help():
    ...
    table.add_row("미 국채 5년", "금리 ≥ 4.50", "[red]빨간색[/]", "긴축 강화/금리 재인상 공포")
    table.add_row("(중기 통화정책 반영)", "4.00 ≤ 금리 < 4.50", "[orange3]주황색[/]", "중립 상단/불확실성 상존")
    table.add_row("", "3.50 ≤ 금리 < 4.00", "[green]초록색[/]", "안정/적정 수준 유동성")
    table.add_row("", "금리 < 3.50", "[blue]파란색[/]", "금리 급락/단기 유동성 경색 또는 침체 우려")
    table.add_section()

    table.add_row("미 국채 10년", "금리 ≥ 4.80", "[magenta]보라색[/]", "시스템 위기/주식 Valuation 붕괴")
    table.add_row("(글로벌 벤치마크)", "4.50 ≤ 금리 < 4.80", "[red]빨간색[/]", "임계점/고금리 쇼크 (기술주 부담 가중)")
    table.add_row("", "4.20 ≤ 금리 < 4.50", "[orange3]주황색[/]", "고금리 지속/인플레이션 끈적임 경계")
    table.add_row("", "3.80 ≤ 금리 < 4.20", "[green]초록색[/]", "골디락스/적정 성장과 물가의 균형점 (뉴노멀)")
    table.add_row("", "3.30 ≤ 금리 < 3.80", "[yellow]노란색[/]", "수요 둔화/금리 인하 선반영")
    table.add_row("", "금리 < 3.30", "[blue]파란색[/]", "침체 확정/안전 자산 선호 (Flight to Quality)")
    table.add_section()

    table.add_row("미 국채 30년", "금리 ≥ 5.00", "[magenta]보라색[/]", "재정 적자 심화/기간 프리미엄 급증")
    table.add_row("(장기 기대 인플레)", "4.60 ≤ 금리 < 5.00", "[red]빨간색[/]", "장기 인플레 우려/국채 발행 부담")
    table.add_row("", "4.00 ≤ 금리 < 4.60", "[green]초록색[/]", "장기 안정/수급 균형")
    table.add_row("", "금리 < 4.00", "[blue]파란색[/]", "장기 저성장/디플레이션 우려")
    table.add_section()
    ...
    table.add_row("QGRD 스마트그리드 지수명", "고점 대비 낙폭 ≤ 5%", "[red]빨간색[/]", "신고가 랠리 및 초강세")
    table.add_row("", "5% < 고점 대비 낙폭 ≤ 10%", "[orange3]주황색[/]", "건전한 조정")
    table.add_row("", "10% < 고점 대비 낙폭 ≤ 20%", "[yellow]노란색[/]", "기술적 조정기 진입")
    table.add_row("", "고점 대비 낙폭 > 20%", "[blue]파란색[/]", "전력 하락 사이클/침체")
    table.add_section()
