...
def show_help():
    ...
    # [정배열 정의 · 2026-08-29] 5>20>60 에 **현재가>120일선**을 더해 항목 열에서 한 번
    #  정의하고, 조건 열에서는 '정배열'로 줄여 쓴다. 종전 정의는 120일선을 보지 않아
    #  "완벽한 정배열"이라는 라벨과 조건이 어긋났다(성숙한 베어마켓 랠리가 빨강으로 떴다).
    #  실측 근거는 analysis.price_trend_color 주석 참조 — 빨강의 4.6%가 주황으로 내려간다.
    # [폭] '구조'(20일선 vs 60일선)를 항목 라벨에서 한 번 정의하고, 조건에서는 반복하지
    #  않는다. 종전에는 행마다 '20일선<60일선'을 되풀이해 노란색 조건이 두 줄로 접혔다.
    table.add_row("지수값·종목 현재가", "[상승] 정배열 & 20일선 이격도 ≥ 상한", "[magenta]보라색[/]", "정배열 속 단기 과열 (신규 진입만 자제 · 보유는 TS에 맡김)")
    table.add_row("(추세)", "[상승] 정배열 & 20일선 우상향(5봉)", "[red]빨간색[/]", "완벽한 정배열 상승 추세 (홀딩 및 추세 추종 구간)")
    table.add_row(" 구조=20일선vs60일선", "[상승] 위 두 조건에 못 미침 (120일선 아래 포함)", "[orange3]주황색[/]", "상승 구조 속 조정·장기선 미확인 (매수 기회 탐색 구간)")
    # [노랑을 밝기로 가른다 · 2026-08-29] 노랑 하나에 '데드캣 반등'과 '바닥 탈출 초기'가
    #  섞인다 — 둘 다 역배열(20일선<60일선)에서 20일선을 되찾은 모습이라 구조만으로는
    #  갈리지 않는다. 실측(44종목 10년, 노랑 10,794봉)에서 가장 크고 일관되게 가르는 축은
    #  52주 위치였다(60일 전방 +6.91% vs +2.00%, 5개 창 중 4개 부호 일치. 같은 자리의
    #  120일선은 +1.52%p·3/5, ADX·DMI 는 사실상 못 가른다).
    #  데드캣 쪽에 쓰는 하늘색(sky_blue3)은 새 색이 아니다 — 이 팔레트에서 이미 '하락축의
    #  옅은/미확정 단계'다(국면 PendDown '하락 미확정', 추세품질 '미검증', 환율 1200~1300).
    #  자세한 근거는 analysis.price_trend_color 주석 참조.
    table.add_row(" 정배열=5>20>60&120위", "[하락] 현재가>20일선 & 20일선 우상향(5봉)", "[yellow]노란색[/]", "하락 추세 속 되돌림 — 아래 둘이 섞이는 구간")
    table.add_row("", "  └ 52주 위치 60% 이상", "[yellow]노란색[/]", "바닥 탈출 초기 쪽 (60일 전방 +6.9% · 빨강 수준)")
    table.add_row("", "  └ 52주 위치 60% 미만", "[sky_blue3]하늘색[/]", "데드캣 반등 — 하락 미확정 (60일 전방 +2.0% · 최약)")
