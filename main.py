...
def show_help():
    ...
    # [노랑은 한 색으로 두되 모호함을 밝힌다 · 2026-08-29] 노랑 하나에 '데드캣 반등'과
    #  '바닥 탈출 초기'가 섞인다 — 둘 다 역배열(20일선<60일선)에서 20일선을 되찾은 모습이라
    #  구조만으로는 갈리지 않는다. 52주 위치로 쪼개 두 색으로 찍어 봤다가 되돌렸다(실측
    #  격차는 크지만 그 값이 이미 `52W%` 열에 숫자로 있고, 값 색상의 축은 EMA 구조 하나다).
    #  색을 늘리는 대신 **섞인다는 사실을 설명에 적는다** — 색이 두 사건을 합쳐 보여준다는
    #  것을 알고 보는 것과 모르고 보는 것은 다르다. 근거는 analysis.price_trend_color 주석.
    table.add_row(" 정배열=5>20>60&120위", "[하락] 현재가>20일선 & 20일선 우상향(5봉)", "[yellow]노란색[/]", "하락 추세 속 되돌림 (데드캣 반등/바닥 탈출 초기가 섞이는 구간)")
