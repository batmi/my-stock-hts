...
#  갱신 자체는 임시공휴일 반영에 필요하므로 앱이 스스로 한다(modules/holiday_calendar_update):
#    7일마다 백그라운드에서 갱신하고, 갱신 전후 휴장일을 비교해 달라진 날짜를 텔레그램으로 알린다.
#    tools/update_holidays.sh 는 수동 실행용으로만 남겨 둔다(cron 불필요).
