...
class TelegramCommander:
        ...
        try:
            ...
            if result:
                if result.startswith("⚠️"):
                    ...
                    msg = f"{result}\n\n마음에 드는 종목이 있다면 터미널 HTS 메뉴 [7] 관심 종목 관리 -> [2] 관심 종목 추가 를 통해 수동으로 등록해주세요."
    ...
    def _cmd_addrestrict(self, args):
        ...
        # [2026-09-20] _resolve_stock 은 관심종목에 없는 6자리·영문 입력을 '코드'로 받아들이고,
        #  종목명을 못 찾으면 입력값을 이름으로 되돌려 준다. 제한은 안전장치라 오타로 엉뚱한
        #  코드가 등록되면 **의도한 종목은 제한되지 않은 채** '완료'만 남는다 — 이름을 확인하지
        #  못한 등록은 그 사실을 밝힌다(등록 자체는 무해하므로 막지 않는다).
        if str(name or "").strip().upper() == str(code).strip().upper():
            msg += ("\n\n⚠️ 종목명을 확인하지 못한 코드입니다 — 오타라면 의도한 종목은 제한되지 않은 상태입니다. "
                    "/restricted 로 목록을 확인하세요.")
    ...
    def _get_default_keyboard(self):
        ...
        return {
            "keyboard": [
                ...
                [{"text": "📝 관심 종목"}, {"text": "🎯 포지션"}, {"text": "📅 투자 일정"}],
