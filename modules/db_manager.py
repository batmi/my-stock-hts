...
class DBManager:
    ...
    def shift_daily_assets(self, account, amount):
        ...
        [폐기 2026-09-01 — 호출부 없음] 입출금 보정은 이제 원본을 두고 **읽을 때 환산**하는
        쪽(net_transfer + get_max_daily_asset)으로 통일됐다. 이동은 되돌릴 수 없고, 추정이
        틀리면 고점이 낮아져 드로다운을 과소평가한다(= 리스크 한도가 조용히 열린다).
        새 코드에서 부르지 말 것. 남겨 둔 이유는 테스트가 '정당한 입금으로 과거가 함께
        올라간 계열'을 만드는 데 쓰기 때문이다.
