...
[현황 · 2026-09-18] 이 모듈의 웹 로그인 스크래핑 경로는 KRX 약관 위반으로 IP 차단을 받아
 KRX_WEB_SCRAPING_ALLOWED(기본 OFF) 뒤로 물러났다. 아래 get_* 는 먼저 modules/krx_openapi(Open API,
 인증키 하나)를 보고, 게이트가 켜졌을 때만 웹 경로를 탄다. 아래 서술의 KRX_ID/KRX_PW 는 그
 웹 경로에만 해당한다.
