...
def _trading_cycle_items():
    ...
    items = [
        ...
        {"desc": "거래 종료 시간", "help": "매매 허용 종료 시각 (HHMM). 기본값은 KRX 정규장 마감(1530)이며, 종가 단일가(동시호가) 15:20~15:30은 자동 회피하므로 실효 매매는 15:20까지다. KRX 애프터마켓(16:00~20:00)까지 운용하려면 2000 — 15:30~16:00(휴게, 시장 없음)은 설정과 무관하게 닫힌다", "name": "SYSTEM_TRADING_END_TIME", "type": "time", "section": "5-1. 거래 시간·주기",
        ...
        {"desc": "장 종료 후 KRX 정규장 종가 기준", "help": "모든 장(KRX 애프터마켓 20:00)이 끝난 뒤 화면 '현재가'를 KRX 정규장 확정 종가로 고정합니다. 끄면 마지막 실거래가(그날 KRX 애프터마켓 최종가)가 다음 개장까지 그대로 보입니다. 살아있는 시장(NXT 프리 08:00~09:00 · KRX 정규장 · KRX 애프터 16:00~20:00)에서는 설정과 무관하게 그 시장의 현재가를 표시하고, 15:30~16:00(휴게, 시장 없음)은 정규장 종가에 고정합니다. ※ 이 설정은 표시가만 정합니다. 지표는 두 모드 모두 KRX 정규장 종가 봉으로 계산하며(토스 모드는 KRX 포털 일봉의 애프터 최종가 종가를 휴게 캡처·yfinance로 기억한 정규장 종가로 교체), 주문 가격과 손절·트레일링 트리거는 항상 실시간가를 씁니다.", "name": "USE_KRX_CLOSE_AFTER_HOURS", "type": "bool", "choices": ["y", "n"], "section": "5-3. 데이터·통신",
