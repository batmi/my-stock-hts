...
다음은 '{name}({code})' 종목에 대해 가격 노이즈와 체결 오차를 반영하여 10,000회 반복 수행한 몬테카를로 백테스팅 시뮬레이션 결과입니다.

{backtest_info}

이 백테스팅 결과를 바탕으로 다음 항목들을 심층 분석해 주세요:

1. 🎲 [전략 견고성 평가]: 노이즈가 주입된 10,000번의 시뮬레이션에서 수익 발생 확률, 평균 수익률, 그리고 하위 5% 최악의 경우(VaR, CVaR)를 종합하여 이 전략이 시장의 불확실성을 얼마나 잘 견뎌내는지(Robustness) 평가해 주세요. 전략이 운에 좌우되는지 통계적 우위가 있는지 진단해 주세요.
