...
1. 🔍 [과최적화(Overfitting) 진단]: IS(학습 구간) 수익률과 OOS(검증 구간) 수익률의 차이 및 성과 유지율을 분석하여, 현재 설정된 파라미터가 과거 데이터에 지나치게 과적합되었을 가능성이 높은지 진단해 주세요.
2. 📊 [폴드별 성과 및 승률 해석]: 각 폴드의 OOS 승률을 바탕으로, 현재 추세추종 기반의 익절/손절/트레일링스탑 설정이 승률을 희생하고 손익비를 추구하는 형태인지, 아니면 진입 타점 자체가 불리한지 해석해 주세요.
3. 💡 [매매 파라미터 튜닝 조언]: 이 전략을 '{name}' 종목에 더 견고하게(Robust) 적용하기 위해, 매수/매도 허들, 익절/손절폭, 스코어링 가중치(추세/모멘텀 등)를 어떻게 조정해 보는 것이 좋을지 구체적인 가설과 개선 방향을 제안해 주세요.
...
먼저 전체적인 기본 분석을 하고, 이어서 각 보조지표(이격도, MACD, ADX & DMI, RSI, CCI)를 하나씩 개별적으로 상세 분석해 주세요. 각 지표 상세 분석에는 현재 위치/방향, 시그널(과매수·과매도, 골든/데드크로스, 다이버전스 등), 그리고 그것이 주가에 시사하는 바를 구체적으로 담아 주세요.
...
[3. 종합 진단 및 전략]
- 기본 분석과 지표별 상세 분석을 종합한 매수/보유/관망/매도 의견과 근거, 주목해야 할 가격대 및 리스크
"""
