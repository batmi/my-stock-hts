...
# FIELD_MAP_DOMESTIC 및 FIELD_MAP_OVERSEAS_DETAIL 변수는 기존 그대로 유지
FIELD_MAP_DOMESTIC = {
    ...
    if not is_overseas:
        if basic_output:
            ...
            for i in range(0, len(items), 2):
                k1, v1 = items[i]
                label1 = FIELD_MAP_DOMESTIC.get(k1, k1)
                k2, v2 = items[i+1] if i+1 < len(items) else ("", "")
                label2 = FIELD_MAP_DOMESTIC.get(k2, k2) if k2 else ""
                def fmt(v, label):
                    s = str(v).strip()
        ...
        if detail_data:
            ...
            for i in range(0, len(items), 2):
                k1, v1 = items[i]
                label1 = FIELD_MAP_OVERSEAS_DETAIL.get(k1, k1)
                k2, v2 = items[i+1] if i+1 < len(items) else ("", "")
                label2 = FIELD_MAP_OVERSEAS_DETAIL.get(k2, k2) if k2 else ""
                def fmt(v, label):
                    s = str(v).strip()
                    if label.endswith("일자") and len(s) == 8 and s.isdigit(): return f"{s[:4]}/{s[4:6]}/{s[6:]}"
                    try:
