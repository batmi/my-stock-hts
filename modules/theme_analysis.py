...
def analyze_market_trends_with_gemini(custom_prompt=None):
    ...
    if custom_prompt:
        ...
        prompt = f"""
        [현재 시각: {now} (KST)]
        당신은 대한민국 증권가(여의도)에서 가장 실력이 뛰어난 주식 분석 전문가입니다. 
        구글 검색을 통해 실시간 데이터를 수집한 후, 오늘 한국 증시(KOSPI, KOSDAQ)를 주도하고 있는 '핵심 테마 5가지'를 분석해 주세요.

        반드시 다음의 상세 분석 가이드라인을 지켜주세요:

        1. **실시간 테마 분석**: 
           - 단순히 오늘 많이 오른 종목이 아니라, '거래대금'과 '뉴스 모멘텀'이 동반된 주도 테마를 선정할 것.
           - 각 테마별로 왜 오늘 오르는지(공시, 뉴스, 글로벌 증시 연동성 등)를 구체적으로 작성할 것.

        2. **종목 및 수급 분석**:
           - 각 테마별 **대장주 1개와 핵심 관련주 2개**를 명시할 것.
           - 가급적 해당 종목들에 대한 외국인/기관의 실시간 순매수 유입 현황(뉴스/속보 기준)을 언급할 것.

        3. **기술적 전략 및 리스크**:
           - 현재가 기준 '추격 매수' 구간인지, '눌림목 대기' 구간인지 기술적 의견을 줄 것.
           - 해당 테마의 열기가 식을 경우 발생할 수 있는 '리스크 요인(오버행, 재료 소멸 등)'을 경고할 것.

        4. **형식**: 
           - 가독성을 위해 표(Table)와 불렛포인트를 적절히 섞어서 '마켓 인사이트 보고서' 형태로 출력할 것.
        """
