...
def evaluate_market_indicator(name, price, yh_rate=None):
    ...
    status_desc = ""
    if name == "미국채 10년물 금리":
        if price >= 4.80: status_desc = "시스템 위기/Valuation 붕괴"
        elif 4.50 <= price < 4.80: status_desc = "임계점/고금리 쇼크"
        elif 4.20 <= price < 4.50: status_desc = "고금리 지속/인플레 경계"
        elif 3.80 <= price < 4.20: status_desc = "골디락스/적정 성장"
        elif 3.30 <= price < 3.80: status_desc = "수요 둔화/금리인하 선반영"
        elif price < 3.30: status_desc = "침체 확정/안전자산 선호"
    elif name == "미국채 5년물 금리":
        if price >= 4.50: status_desc = "긴축 강화/금리 재인상 공포"
        elif 4.00 <= price < 4.50: status_desc = "중립 상단/통화정책 불확실성"
        elif 3.50 <= price < 4.00: status_desc = "안정/적정 유동성"
        elif price < 3.50: status_desc = "금리 급락/침체 우려"
    elif name == "미국채 30년물 금리":
        if price >= 5.00: status_desc = "재정 적자 우려/기간 프리미엄 급증"
        elif 4.60 <= price < 5.00: status_desc = "장기 인플레 우려/발행 부담"
        elif 4.00 <= price < 4.60: status_desc = "장기 안정/수급 균형"
        elif price < 4.00: status_desc = "장기 저성장/디플레이션 우려"
    elif name == "브랜트유":
    ...
    if yh_rate is not None:
        if name == "SOX (반도체)":
            ...
            elif yh_rate < -20.0: status_desc = "전력 하락 사이클/침체"
        elif name == "NBI (바이오)":
            if yh_rate >= -5.0: status_desc = "신고가 랠리/초강세"
