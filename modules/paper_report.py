"""관찰(페이퍼 트레이딩) 계좌 관리 — 메뉴 5-8.

[역할 분담] 관찰 모드는 api 층에서 잔고·예수금·주문을 가로채므로
...
def show_paper_menu():
    ...
    if not paper_broker.is_active():
        config.console.print(
            ...
            "[dim]현재 모드에서는 5-3(트레이딩 상태)·5-4(트레이딩 평가)·9(자산 관리)를 그대로 쓰시면 됩니다.[/dim]")
        utils.pause()
        return
...
def _print_status():
    ...
    config.console.print(
        "[dim]※ 체결 내역·일별 성과는 [5-4] 트레이딩 평가, 잔고·평가손익은 [9] 자산 관리에서 "
        "그대로 확인할 수 있습니다.[/dim]")
