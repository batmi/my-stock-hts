# modules/manage/watchlist.py
"""[7] 관심 종목 관리 (기존 modules/manage.py).

외부 인터페이스는 패키지(__init__)가 재수출한다.
"""
import logging
...
def _pkg():
    """패키지(modules.manage) 네임스페이스 접근자 — patch('modules.manage.X') 의미론 보존용."""
    import modules.manage as _mg
    return _mg
...
def manage_stock_menu():
    ...
    while True:
        ...
        if choice == "1":
            ...
            if _pkg().get_current_price(mode='add') is not False: is_success = True
        elif choice == "3":
