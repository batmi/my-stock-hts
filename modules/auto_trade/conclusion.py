...
class ConclusionMonitor:
    ...
    def _register_ws_exec_callback(self):
        ...
        WS가 꺼져있거나 KIS의 HTS ID가 미설정이면 콜백이 호출되지 않으므로,
        기존 주기 폴링이 그대로 체결을 처리한다(완전 폴백).

        [2026-09-09] 토스(mode 3)도 여기에 붙는다 — 주문 이벤트(personal:order) 웹소켓이
         열렸다. KIS 체결통보와 같은 자리·같은 규약(내용을 믿지 않고 깨우기만)이다.
        """
        try:
    ...
    def _on_ws_exec_notice(self, notice):
        ...
        [주의] KIS의 구독 키는 HTS ID라 같은 ID의 **다른 계좌** 주문도 여기로 들어온다.
          지금은 '깨우기'만 하므로 무해하지만(폴링이 우리 계좌만 본다), 통보 내용을 쓰기
          시작하면 notice['acnt'](토스는 notice['acnt_seq'])로 계좌를 갈라야 한다.

        [재동기화 · 토스] 토스 주문 채널의 무손실 보장은 **연결 세션 안에서만**이라,
          끊겨 있던 동안의 이벤트는 다시 오지 않는다. 그래서 피드가 재연결할 때마다
          event='RECONNECT' 통보를 한 번 보낸다 — 여기서 하는 일은 같다(깨우기).
          이 경로가 있어야 재연결 직전에 체결된 주문이 다음 주기까지 방치되지 않는다.
        """
        try:
