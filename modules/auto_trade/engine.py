...
class DefaultStrategy:
    ...
    def analyze_sell(self, code, name, df, current_price, buy_price, profit_rate, thresholds=None, already_half_sold=False, holding_days=0, is_mr_holding=False, highest_price=0.0):
        ...
        # [체인을 소비하지 않는다] 종전에는 `elif ts_msg:` 였다. 시간청산 조건이 참인데
        #  **유예**되면 reason 이 빈 채로 체인이 끝나, 그날 트레일링 스탑 판정이 통째로
        #  건너뛰어졌다 — 승자가 무너지는 순간 청산을 미루는 방향이다. 위 반익절 절이
        #  같은 형태로 이미 한 번 고쳐졌는데(그 주석 참조) 여기만 남아 있었다.
        #  다른 절들은 걸리면 reason 을 채우므로 이 후치 판정은 종전과 동등하고,
        #  **유예된 경우만** 새로 TS 를 볼 수 있게 된다.
        #  [실측 2026-09-01] 백테스트 10년에서 삼킨 사례 0건이고
        #   TIME_STOP_MIN_PROFIT_RATE 를 5·10 으로 올려 유예를 146·417건까지 늘려도 0이다
        #   (유예 조건 '최근 5일 고점 ≥ 10일 고점'과 TS 발동 조건 '고점 대비 콜백만큼 하락'이
        #    거의 배타적이다). 그래도 그 안전이 유예 조건의 형태에 기대고 있어 함정을 없앤다.
        if not reason and ts_msg:
            reason = ts_msg
...
class OrderManager:
    ...
    def send_order(self, code, qty, type_str, name=None, profit_amt=0, profit_rate=0.0, reason=None, score=0, price=0, rule=None, stop_loss_rate=0.0, buy_price=0.0, cost_amt=0.0):
        """주문 전송 및 상태 등록"""
        ...
        try:
            ...
            if res_json['rt_cd'] == '0':
                ...
                db_manager.db.insert_trade(f"{type_str}(AUTO)", code, name, qty, str(price), odno, snapshot=snapshot, profit_amt=profit_amt, profit_rate=profit_rate, reason=reason, score=score, stop_loss_rate=stop_loss_rate, buy_price=buy_price, cost_amt=cost_amt)
...
class RiskManager:
    ...
    def current_risk_scale(self, market_type=None):
        ...
        신규 진입 사이징과 히트 캡(SYSTEM_MAX_PORTFOLIO_RISK)에만 적용되며 청산 로직에는
        관여하지 않는다. 미산출 시 1.0(축소 없음).

        [어디에 곱해지나 · 2026-09-01 정정] 사이징에서는 **기초 비중**에 곱한다
        (allocate_budget 의 `scaled_ratio = invest_ratio * risk_scale`). 종전 이 자리에는
        'SYSTEM_RISK_PER_TRADE 에 적용된다'고 적혀 있었는데, 그 층은 최종액을 정하는 일이
        없어(실측 2026-09-01, 10년·사이징 421회에서 **구속 0회** — 변동성 층 99.0% ·
        기초비중 1.0%) 거기에만 곱하면 배수가 사실상 무력했다. 기초 비중에 곱하면 변동성
        층의 상한(기초 × 배수)도 함께 내려가 곧바로 방어가 걸린다. 자세한 경위는
        allocate_budget 의 [리스크 스케일링] 주석에 있다.
