...
class AutoTrader:
    ...
    def _update_risk_scale(self):
        ...
        (VOLATILITY_SCALING_MIN 0.4 바닥 = 기초비중 25%의 40% = 자산의 약 10%), 리스크층 상한은
        그보다 항상 커서 바인딩되지 않는다. 그래서 이 배수를 곱해도 **종목당 배분액은 배수가
        약 0.45 미만으로 내려가기 전까지 1원도 변하지 않는다** — 단일 트리거(PendDown 0.6 /
        휩소율 0.6 / DD-5% 0.75 / DD-10% 0.5)로는 도달하지 못하고 둘 이상 겹쳐야(0.36) 걸린다.
        실효 방어는 **히트 캡 경로**에서 나온다: effective_portfolio_cap = 캡 × 배수이므로
        4슬롯 보유(히트 약 5.6%) 기준 배수 0.5 이하부터 신규 매수가 예산 초과로 차단된다.
        즉 '단계적 축소'가 아니라 사실상 on/off 차단 스위치로 동작한다.
        (검증: 일별 배수를 이식한 백테스트에서 '스케일링 OFF'와 현행이 거래 803건까지 동일.
         변동성 캡에도 배수를 곱하면 의도대로 축소되나 3년 수익 -26%·MDD 25% 완화로 미채택.)"""
