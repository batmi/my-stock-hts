...
AXES = {
    ...
    # [주의] 이 축은 2026-08-17에 5 → 10으로 채택됐다가 **2026-09-09에 5로 되돌아왔다**
    #  (계측기 3건 수정 후 재측정에서 36-0-39로 역전 — config.py OBV_MA_PERIOD 주석 참조).
    #  기준선을 None(=현행 config)으로 두면 기준선과 '5' 팔이 같아져 0-N-0으로 나온다.
    #  그래서 세 값을 모두 명시한다 — 재실행 때 옛 값과 새 값을 그대로 맞대볼 수 있어야 한다.
    "C": ("OBV 이평 기간", [
        ("[기준선] 5 (현행)", {"OBV_MA_PERIOD": 5}),
        ("10 (2026-08-17~09-09 채택분)", {"OBV_MA_PERIOD": 10}),
        ("20", {"OBV_MA_PERIOD": 20}),
    ]),
    "D": ("거래량 이평 기간", [
        ("[기준선] 20", None),
