...
- **KRX after-market (16:00–20:00), NXT pre-market & SOR support** — from 2026-09-14 KRX runs an after-market session 16:00–20:00. Quotes and orders use the KRX after-market (orders via SOR; **ETF/ETN are not traded after hours — common stock only**); **the NXT-only after-hours window (15:30–16:00) is not used** — the auto-trader and reserved orders stay closed for those 30 minutes and manual orders require confirmation. Pre-market (08:00–08:50) keeps using NXT quotes and orders. Trading pauses during the KRX single-price auctions (08:50–09:00 and 15:20–15:30). **The auto-trader's default window is the KRX regular session, 09:00–15:30** (all indicators and daily bars are on a KRX basis). To run through the after-market, set the trading end time to `2000` under `[0] → 5-1` (both entries and exits run until then).
