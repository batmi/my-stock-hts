...
export JOURNAL_BOT_LABEL=""                         # optional (display name, auto-generated by default)
...
The HTS reports **every** fill on its accounts. Orders you placed by hand in the Toss app or your broker's own HTS are detected by balance reconciliation and uploaded too. Lumping all of them under "system" would merge automated performance with discretionary trading.

Order origin decides which is which (`isSystem`).

| Order origin | `isSystem` | Class assigned by the server |
|---|:---:|---|
| `(AUTO)` — placed by AutoTrader on a strategy signal | `true` | Pinned to `시스템` (system) |
| `(예약)` — a reserved order that triggered | `false` | Fallback below |
| `(수동)` — placed by hand from this HTS's menus | `false` | Fallback below |
| `(외부)` — placed in a mobile app / broker HTS and detected here | `false` | Fallback below |
| No tag — origin unknown | *(not sent)* | Fallback below |

Reserved orders execute unattended, but **a human set the condition**, so they do not count as system trades.

**Fallback**: when the class cannot be determined, the HTS omits `tradeClass` entirely. The server inherits the class from the most recent record for the same symbol (e.g. `장기투자` / long-term), and if there is none it stays empty for you to pick on the web. The `시스템` class is **never** inherited — an earlier version stored every HTS-originated record as `시스템`, so inheriting it would make that contamination permanent.

> Records already stored as `시스템` stay as they are. The server dedupes by `brokerExecutionId`, so even a resync will not overwrite them — fix those on the web.
