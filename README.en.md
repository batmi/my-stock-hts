...
- **KRX regular-session close after hours** (`USE_KRX_CLOSE_AFTER_HOURS`, ON by default) — once every session has closed, the current price is pinned to the KRX regular-session close. Turn it off to keep showing that day's last KRX after-market print until the next open. This setting only affects the displayed price — indicators use KIS daily bars (confirmed regular-session close) in KIS mode and official KRX daily bars (which include after-market prints) in Toss mode, and **order prices always use the live price.**
...
- **Circuit breaker (CB) and VI alerts** based on exchange status flags. VI is monitored in both the KRX regular session and the after-market (16:00–20:00); CB exists only in the regular session and is checked there only.
...
│   ├── krx_daily.py        # Domestic daily bars (pykrx/FDR, official KRX — after-market prints included since 2026-09-14)
...
- **Backtest/live data parity** — domestic backtests always use official KRX data (pykrx/FDR) regardless of mode; since 2026-09-14 official KRX daily bars include after-market prints, so bars from that date on differ slightly in close/volume from KIS-mode live trading (KIS regular-session bars) while matching Toss-mode live trading. A **warning** is raised when the retrieved window is shorter than requested, so truncation is never silent.
