...
def test_run_backtest_optimization_grids(mock_ask, mock_get_data, mock_name, mock_val):
    ...
    #   6(직접입력) -> 005930 -> y(설정변경) ->
    #   [기간]100 -> [매수점수]8.0 -> [매수RSI상한]60 -> [TS발동]10.0 -> n(시장필터X) ->
    #   n(피라미딩 변경X) -> n(가중치 변경X) -> 1(모드=단일) -> n(AI진단X) -> q(메인복귀)
    #
    # [주의] 익절률·반익절·익절RSI·매도점수·TS콜백·시간청산·ATR·손절은 설정 메뉴에서
    #  봉인(BACKTESTED_HIDDEN_KEYS)되면서 백테스트 조건 변경에서도 더 이상 묻지 않는다.
    #  답변 개수가 실제 프롬프트 수와 어긋나면 뒤쪽 답이 한 칸씩 밀려 실행 모드 선택이
    #  '1'이 아닌 값을 먹고, res 미할당(UnboundLocalError)으로 터진다.
    mock_ask.side_effect = [
        "6", "005930", "y", "100", "8.0", "60", "10.0", "n", "n", "n", "1", "n", "q"
    ]
